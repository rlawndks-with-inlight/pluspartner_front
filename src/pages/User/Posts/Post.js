...
border-bottom: 16px solid #7030A0;
...
const Post = (props) => {
    ...
    const returnTitle = (ttl) => {
        if (postTable == 'notice') {
            return "pluspartners - 플러스파트너 : 회사명 - 공지사항 / " + ttl;
        } else if (postTable == 'issue') {
            return "pluspartners - 플러스파트너 : 회사명 - 핵심이슈 / " + ttl;
        } else if (postTable == 'theme') {
            return "pluspartners - 플러스파트너 : 회사명 - 핵심테마 / " + ttl;
        } else if (postTable == 'feature') {
            return "pluspartners - 플러스파트너 : 회사명 - 특징주 / " + ttl;
        } else if (postTable == 'oneevent') {
            return "pluspartners - 플러스파트너 : 회사명 - 하루1종목 / " + ttl;
        } else if (postTable == 'oneword') {
            return "pluspartners - 플러스파트너 : 회사명 - 하루1단어 / " + ttl;
        } else if (postTable == 'strategy') {
            return "pluspartners - 플러스파트너 : 회사명 - 전문가칼럼 / " + ttl;
        } else {
            return "pluspartners - 플러스파트너 : 회사명";
        }
    }
    useEffect(() => {
    ...
    const handleShare = () => {
        if (navigator.share) {
            navigator.share({
                title: post.title,
                url: 'https://pluspartners.kr' + location.pathname,
            });
        } else {
